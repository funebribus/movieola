class FilmesController < ApplicationController
  ...
  def update
    respond_to do |format|
      if @filme.update(filme_params)
        @filme.attributes = {'ator_ids' => []}.merge(params[:filme] || {})
  ...
  private
    ...
    def set_filme
      ...
      @topico = @filme.topicos.build
    end

    # Never trust parameters from the scary internet, only allow the white list through.
    def filme_params
      params.require(:filme).permit(:titulo, :datalancamento, :imagemdecapa, :sinopse, :duracao, :tags, :nota, {:ator_ids => []}, {:genero_ids => []}, {:diretor_ids => []}, {:estudio_ids => []}, {:produtor_ids => []})
    end
end
